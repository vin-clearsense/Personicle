...
#### EPISODIC EVENTS & HEALTH MAINTENANCE/SCREENING PROTOCOLS

Many interesting events about a person may be imported from their calendar, email, or social media.  For health applications, a major source maybe EHR listing their diagnostic tests, doctor’s opinions, medical prescriptions and other health related events and observations. While hospital based EMR systems are a large proportion of healthcare data is generation, outpatient clinical settings provide a trove of biographical data that is extremely interelated with the patients current health status. Recent trends in procedural and medical advancement have promoted hospitals and private practice models to begin sprouting and are providing an easy source for more and more patients to recieve maintenance healthcare, building new and widely updated records of their health status. While inpatient hospitals have a wealth of healthcare information, the frequency of the same patient visiting these sites for care can diminish over time if the specialized services are not required. This promotes the use of outpatient facilities which see patients more frequently and thus have data that is routinely being updated. Specifically the United States Preventative Services Taskforce (USPSTF) guidelines for health maintenance screening extends through almost all paradigms of physical and mental health - which is strictly adhered to in all primary clinics. These features make outpatient data relatively useful for organizing a health timeline in a personicle for preventative measures while being enforced by research backed predictive models which these screening guidelines are based on to create a nuanced portolio of individual health status. The pathologies that the USPSTF have developed guidelines for are also the largest proportion of successfully medically managed conditions. Interconnecting this source of data with patient screening incidence can provide an important source of relevant biophysical status of an individual and a population. Thereby sources from both inpatient and outpatient visits that provide an episodic picture should be imported in the Personicle and should be used to assimilate with data streams to build and enrich the Personicle.

|Resources|Link|
|----|-------|
|United States Preventative Screening Task Force Guidelines | https://www.uspreventiveservicestaskforce.org/uspstf/recommendation-topics/uspstf-and-b-recommendations |
