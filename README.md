...
### INTERWEAVING CLINICAL RESEARCH 

The basis for modern medicine guidelines lies in the foundation of data. In particular research published everyday has continued to have an impact on the indications for procedures, using new medications and growing the database of potential new mechansims to solve everyday health problems. What is extremely difficult and often labor intensive is to determine which research methods are valid enough to be followed. The synthesization, testing of validity and comparison against other contrary results on the same question is a part of the process to develop new guidelines and given the specificity and unique considerations in each topic - there is an inability to proper automate this. Yet, once guidelines are established, being able to disseminate those to enforce health maintenance is a problem of information dissemination. The topics for screening guidelines for example are done to cover the most common conditions and preventable ailements. To have routine screening has been a difficult problem to solve in areas where awareness is low due to either low access and low resources. Integrating the most up to date clinical research on screenings with Personicle's as a data stream for example can assist persons in screening reminders without having to first see a physician to be assessed for those. This can enforce seeking medical care whose access can be efficiently assessed when combined with assessing a communities level and locations of actions.

Translational clinical research has been at a forefront of this process but getting the research to inform patients own awareness of their health has been a difficult problem given that there is no viable method to communicate the complexity to patients in terms that they can understand. While the recent COVID pandemic has shown that mobilization of media resources with governmental initiatives have quickly increased awareness for the vaccine and wear to access them - with news of any new updates on varients or vaccine requiements being communicated within minutes - this process is still incredibly resource intensive. Simply dissemination of information for one specific treatment for one condition requires many resources. Every single individual may have multiple indications for treatments or screening protocals but knowing that they actually require them can go amiss. With Personicle's being able to provide an aggregate picture of a patients health status, being able to drastically lower the time from new research dissemination to the patients hands so that they can act on it can allow for an effective automation of clinical research backed decision maintenance for ones health. 

Most importantly, leveraging Personicle's data immediately generate a user specific set of reccomendations everytime new validated research is starting to be implemented. Such data can also be extended into reccomendations to better quality of life goals, maintaining co-morbid conditions to prevent chronic complications all to empower the user take ownership of their health in an informed manner. In the following systemic meta-analysis that can be found here - https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4895829/: research has shown that the single greatest factor for patients to adhere to new healthcare guidelines is their willingness and level to which they participate in adhereing to them. But it is not reasonable to force patients to understand the nuances and positive effects that this can have on thier lives. Thereby connecting Personicles aggregate picture of a user to a clinical sources of data that have proven certain changes can be right specific for them - a personalized evidenced based class of reccomendations will be developed.

The final part of this is a protocal for implementation of these reccomendations. As soon as published research on reccomendations are shared, so to expert commentaries and quality based research begin on optimal solutions for implementation. These will also serve as a source of data in conjunction with evidence based methods to provide users with methods for best implementations and the processes to track progress simultenously. This specific data source of clinical research thereby allows us to turn Personicle's accurate picture of a person into actionable steps; making it easier for users to take accountability and control of their own health without haphazard steps that may not prove to provide any benefit. 
